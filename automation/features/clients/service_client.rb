...
class ServiceClient
    ...
    def get_base_uri
      return YAML.load(File.read("#{$PROJECT_ROOT}/features/clients/config/order.yml"))[ENV['ENVIRONMENT']]["base_uri"]
    end

    def get_mock_base_uri
      return YAML.load(File.read("#{$PROJECT_ROOT}/features/clients/config/order.yml"))[ENV['ENVIRONMENT']]["mock_base_uri"]
    end
